...
//const input = 'https://5b44cf20b0388.streamlock.net:8443/live/ngrp:live_all/playlist.m3u8';
async function fps2png() {
	const input = 'https://5b44cf20b0388.streamlock.net:8443/live/ngrp:live_all/playlist.m3u8';
	console.log('fps2png');
	const filename = 'test-%d.png'
	const output = path.join(fixturesPath, filename)
	await extractFrame({
		log: console.log('image processing...'),
		input,
		output,
		offest: 10000
	}).then(function () {
		console.log('await done');
		PythonShell.PythonShell.run(path.join(__dirname,'text.py'), optionsP, function (err, results) {
			if (err) throw err;
...
function getNumber(data) {
	...
	if(jsonObj.objects!=null){
		result = getCount(jsonObj.objects);
  ...
  //  var query = funcexhib.find({}).select('')
    ...
    var msg = req.query.msg || req.body.msg;
        ...
        if(msg == 'exhibit1'){
          ...
          funcexhib.find()
                        .select('title data detaildata guidedata exhibitpeople exhibitsize exhibitname exhibitdetaildata exhibitpicture -_id')
